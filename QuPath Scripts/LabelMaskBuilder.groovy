...
/**
 * Script con metodi di utility per estrarre una label mask e una tabella in formato TSV da annotazioni manuali in
 * QuPath.
 *
 * Argomenti:
 * - Posizione 0: boolean separateNuclei -> se false, ignora i nuclei e il labelling è uniforme per l'intera cellula
 */
...
static void saveMask(@NotNull BufferedImage labelImage, @NotNull String formatName, @NotNull String path) {
    try {
        ...
        System.out.println("✅ Manual segmentation label image saved (16-bit) to: $path")
    } catch (Exception e) {
        System.out.println("❌ Failed to save label image: ${e.message}")
    }
}

/**
 * Restituisce una mappa da label image a label table corrispondente. Se {@code separateNuclei = false} i pixel
 * corrispondenti ai nuclei nell'immagine finale avranno label uguale a quella della loro cellula genitrice e
 * nella tabella non compariranno record per le annotazioni di tipo nucleo.
 *
 ...
 * @param separateNuclei Il flag booleano che indica se i nuclei avranno label diversa dalla cellula genitrice. Il
 * valore di default è {@code true}
 ...
 */
static @NotNull Map buildMaskAndTable(List<PathObject> annotations, int w, int h, boolean separateNuclei = true) {
    ...
    // Se separateNuclei è false, rimuovi i nuclei dalla lista
    // prima di disegnare — verranno inclusi nell'area del citoplasma padre
    if (!separateNuclei) {
        sorted = sorted.findAll { ann ->
            !( ann.getPathClass()?.getName()?.toLowerCase()?.contains("nucleo") )
        }
    }

    for (ann in sorted) {
        if (label > 65535) {
            System.out.println("Numero etichette disponibili superato. Mi fermo a 65535 annotazioni (16 bit)")
            break
        }
        ...
        if (shape == null) {
            System.out.println("Salto annotazione con forma nulla")
            continue
        }

        // Se separateNuclei è false, passa una lista vuota
        // così subtractChildren non sottrae nulla e la cellula è intera
        def childShapes = separateNuclei
                ? ann.getChildObjects()
...
// Estrae i parametri da CLI
boolean separateNuclei = args.length != 0 ? args[0].toBoolean() : true
print "Estrazione annotazioni"
println separateNuclei ? " separando i nuclei dal citoplasma" : " ignorando i nuclei"
...
def result = buildMaskAndTable(annotations, w, h, separateNuclei)
